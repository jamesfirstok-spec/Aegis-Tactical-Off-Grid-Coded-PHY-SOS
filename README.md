# Aegis-Tactical---Off-Grid-Coded-PHY-SOS
Off-grid Bluetooth Coded PHY SOS system with AES-256 encryption. Fully architected and coded by Gemini 3 Pro AI. (Archived/No Maintenance).

🛡️ Aegis Tactical - Off-Grid Coded PHY SOS

Aegis is a decentralized, peer-to-peer emergency communication system designed for Android. It leverages the Bluetooth 5.0 Long Range (Coded PHY) specification to transmit encrypted SOS telemetry over extreme distances (1km+ Line-of-Sight) without reliance on cellular networks, Wi-Fi, or satellites.
...
Tooling Constraints: This app was built entirely on a smartphone using AndroidIDE. The environment is currently limited to Android 14 (SDK 34), preventing further modernization without migrating to a PC workflow.

Scope Complete: The project successfully met its design goals as a functional prototype.

Personal Bandwidth: I am moving on to other projects.
...
Step 1: User A shows QR Code -> User B scans it. (Now B can hear A).

Step 2: User B shows QR Code -> User A scans it. (Now A can hear B).
...
Install on two compatible Android devices.

Permissions: Grant Location (for Bluetooth scanning), Camera (for QR), and Notifications.

Boot: Press the "BOOT SYSTEM" button to initialize the radio engine.

Pair: Perform the Mutual Pairing (described above).

Test: Walk 500m away and press SOS.
